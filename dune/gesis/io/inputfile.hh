...
#ifdef FILTER_UNKNOWN_CODE
                // TODO: Ask Ronnie: What is this good for?
                int status=convert_well_pumping_data(domain_data.virtual_gridsizes[dim-1],this->setups[nSetups].wdlist,this->setups[nSetups].pdlist);
                if(status>0){
                  logger<<"Setup #"<<nSetups+1<<" : "<<status<<" well data changed into pumping data (gridsize = "<<domain_data.virtual_gridsizes[dim-1]<<")!"<<std::endl;
                }
                

#ifndef WELL_FRACTURE_MODEL_OFF
                logger<<"SIMPLE WELL MODEL USED!!!!"<<std::endl;  // TODO: Ask Ronnie: What is the simple well model?
                if(helper.rank()==0)
                  std::cout<<"SIMPLE WELL MODEL USED!!!!"<<std::endl;

                status=well2pumpingdata_for_simple_well_model(domain_data,this->setups[nSetups].wdlist,this->setups[nSetups].pdlist);
                if(status>0){
                  logger<<"Setup #"<<nSetups+1<<" : "<<status<<" wells for simple well model!"<<std::endl;
                  if(helper.rank()==0) 
                    std::cout<<"Setup #"<<nSetups+1<<" : "<<status<<" wells for simple well model!"<<std::endl;
                }
#endif

#endif // FILTER_UNKNOWN_CODE
