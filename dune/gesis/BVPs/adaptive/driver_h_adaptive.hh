...
void adaptiveDrive_h_rgv( GRID& grid,
  ...
  //std::vector<Dune::shared_ptr<VCType_TP> > solution_hierarchy;
  //#endif
  ...
  for(UINT step=0; step<maxsteps+1; step++){
    ...
    if(step<=maxsteps){
      ...
      */
      ...
      std::stringstream indexrangefile2;
      indexrangefile2 << dir.vtudir << "/elementorder_TPE_step" << step;
      //outputGridviewIndexToDGF( *pgv_tp, inputdata, indexrangefile2.str() );
      outputGridviewIndexToDGF( gv_tp, inputdata, indexrangefile2.str() );
    }


    // gfs_gw and vc_h updated after each refinement step!
    // DARCY_FLUX_DGF darcyflux_dgf( gwp_fwd, gfs_gw, vc_head, baselevel );

    // new plot of velocity field:
    /*
    std::stringstream vtu_q;
    vtu_q << dir.vtudir << "/q_step" << step;
    Dune::Gesis::VTKPlot::output_dgf_to_vtu( gv_gw, 
                                                    gfs_gw, 
                                                    darcyflux_dgf, 
                                                    vtu_q.str(), 
                                                    "Darcy_Flux", 
    ...
    */
    ...
    if( step > 0 ){
      ...
      vtu_m0_file_before_solve  << dir.vtudir << "/test_m0_before_step" << step;
    ...
    vtu_m0_file  << dir.vtudir << "/test_m0_step" << step;
    VTKPlot::output2vtu( gfs_tp,
                         vc_m0,
                         vtu_m0_file.str(),
                         "m0_orig",
    ...
    */
    ...
    //P0GFS p0gfs(*pgv_tp,p0fem);
    P0GFS p0gfs(gv_tp,p0fem);
#if defined L2ProjectionOfM0_OFF
    /*
    VCType_GW vc_m0_gw( gfs_gw, 0.0 );
    CoarseGridP0Projector<GFS_TP,
                          GFS_GW,
                          IDT,
                          dim> sp_ccfv(gfs_tp,gfs_gw,inputdata);
    sp_ccfv.apply( vc_m0, 
                   step,
                   0, 
                   vc_m0_gw );

    General::logMinAndMax( vc_m0_gw, gv_gw.comm() );

    std::stringstream vtu_m0_gw_file;
    vtu_m0_gw_file  << dir.vtudir << "/test_m0_gw_step" << step;
    VTKPlot::output2vtu( gfs_gw,
                         vc_m0_gw,
                         vtu_m0_gw_file.str(),
                         "m0_orig_gw",
                         ...
                         0 );
    */
    U0Type vc_m0_P0(p0gfs,0.0); // local error indicator

    CoarseGridP0Projector<GFS_TP,
                          P0GFS,
                          IDT,
                          dim> sp_ccfv_p0(gfs_tp,p0gfs,inputdata);
    sp_ccfv_p0.apply( vc_m0, 
                      0,
                      0, 
                      vc_m0_P0 );
    
    //General::logMinAndMax( vc_m0_P0, *pgv_tp.comm() );
    General::logMinAndMax( vc_m0_P0, gv_tp.comm() );

    /*
    std::stringstream vtu_m0_P0_file;
    vtu_m0_P0_file  << dir.vtudir << "/test_m0_P0_step" << step;
